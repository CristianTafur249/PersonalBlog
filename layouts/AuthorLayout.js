...
export default function AutorLayout({ children, frontMatter }) {
    const { name, avatar, lengs, ocupation, email, twitter, linkedin, github } = frontMatter
                    ...
                    <div className=' mx-auto mt-10 grid   grid-cols-1 gap-y-16 gap-x-8 pt-10 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-2'>
                        <div className=" mx-auto my-auto flex-col justify-items-center pt-8">
                            ...
                            <div className="m-auto tracking-widest">
                                <p className="m-auto text-center dark:text-white text-gray-700"> Lenguajes manejados:</p>
                                {lengs.map((leng) => (
                                    <a key={leng} className="transition ease-in-out duration-700 hover:border-b-2 relative z-10 rounded-full m-auto ml-3 my-4 p-2 text-center  border-green-700 text-green-700 hover:border-gray-800 hover:text-blue-800 dark:hover:border-gray-50 dark:hover:text-blue-50 ">{leng}</a>
                                ))}
                            </div>
