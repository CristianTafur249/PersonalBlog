...
const SITE_URL = process.env.SITE_URL || 'https://ejemplo.com';
const MAX_URLS_PER_SITEMAP = 7000;
...
(async () => {
    try {
        ...
        for (let i = 0; i < sitemapCount; i++) {
            const sitemapUrls = filteredUrls.slice(i * MAX_URLS_PER_SITEMAP, (i + 1) * MAX_URLS_PER_SITEMAP).join('');
        ...
        const sitemapIndexContent = `
            ...
            <sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
                ${sitemapFiles.map(file => `<sitemap><loc>${SITE_URL}/${file.replace('public/', '')}</loc></sitemap>`).join('')}
